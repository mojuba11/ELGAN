...
const LoginPage = ({ setUser }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 3. Role-based Navigation with explicit handling
      if (user.role === 'manager') {
        navigate('/manager');
      } else {
        // Defaults to fleet for any other role to prevent routing dead-ends
        navigate('/fleet');
      }
    } catch (err) {
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-50 px-4 font-sans selection:bg-[#0089A3] selection:text-white">
      <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-md border border-slate-100">
          ...
          >
            {isLoading ? (
              <>
                ...
                <span className="uppercase tracking-widest text-xs font-bold">Verifying...</span>
              </>
            ) : (
        ...
        <div className="mt-12 pt-6 border-t border-slate-100 text-center">
          ...
          <p className="text-slate-300 text-[8px] mt-2 font-medium italic leading-relaxed">
