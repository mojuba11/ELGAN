    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (err.response?.status === 401) {
                alert("Security session invalid. Redirecting...");
                localStorage.clear();
                window.location.href = '/login';
            } else {
        ...
        <div className="bg-slate-50 min-h-screen p-4 md:p-8 font-sans">
            <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden">
                <div className="bg-[#0089A3] p-6 flex justify-between items-center text-white">
                    ...
                    <button onClick={() => navigate('/fleet')} className="text-white/80 hover:text-white flex items-center text-xs font-bold transition-colors uppercase tracking-widest bg-white/10 px-3 py-2 rounded-lg">
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8">
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Reporting Period</label>
                        <input type="month" name="reportMonth" value={formData.reportMonth} onChange={handleChange} required className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-[#0089A3] font-bold text-slate-700 transition-all" />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input type="number" name="totalIncome" placeholder="0.00" value={formData.totalIncome} onChange={handleChange} required className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-[#0089A3] font-black text-2xl text-slate-800 transition-all" />
                        </div>
                    </div>

                    <div className="bg-cyan-50 border border-cyan-100 p-6 rounded-3xl flex justify-between items-center">
                        <div>
                        ...
                        <div className="bg-white p-3 rounded-full text-[#0089A3] shadow-sm">
                            <Calculator size={28} />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="remarks" rows="3" placeholder="Notes for management reconciliation..." onChange={handleChange} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-[#0089A3] font-medium text-slate-600 transition-all"></textarea>
                    </div>

                    <button type="submit" disabled={isLoading} className="w-full bg-[#0089A3] text-white p-5 rounded-2xl font-black uppercase tracking-[0.2em] hover:bg-[#006F85] transition-all flex items-center justify-center shadow-lg shadow-cyan-100 disabled:bg-slate-300 active:scale-95 text-lg">
                        {isLoading ? <><Loader2 className="animate-spin mr-2" size={20} /> <span>Processing...</span></> : "Submit"}
                    </button>
                </form>
            </div>
