  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/login" element={!user ? <LoginPage setUser={setUser} /> : <Navigate to={user.role === 'manager' ? '/manager' : '/fleet'} />} />

        {/* Protected Fleet Route */}
        <Route path="/fleet" element={user?.role === 'fleet' ? <FleetDashboard user={user} /> : <Navigate to="/login" />} />
